...
***** Fullscreen mode
      ...
      on/off. Then I execute it once so that the default is for full
      screen.
...
******* Org Directory
	...
	The variable 'sb:doc-directory must be defined before
...
(defun sb:orgdircat (filename)
  (concat sb:doc-directory "/org/" filename))
#+end_src
	  ...
	  (setq org-capture-templates
	    '(("t" "Todo" entry (file+headline (sb:orgdircat "capture.org") "Tasks")
              ...
              ("j" "Journal" entry (file+datetree (sb:orgdircat "journal.org.gpg"))
                   "* %?\nEntered on %U\n  %i\n  %a")
              ("i" "InvLim Journal" entry (file+datetree (sb:limdircat "il_journal.org.gpg"))
                   "* %?\nEntered on %U\n  %i\n  %a")
